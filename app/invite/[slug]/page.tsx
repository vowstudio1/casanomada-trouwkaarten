...
function BloomCard({
  children, withCornice = false, color,
  style: extra,
}: {
  ...
  return (
    <div style={{
      ...
      {withCornice && <>
        ...
        <img src={B.cornice_ink}  alt="" aria-hidden style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "contain", opacity: 0.10, filter: inkFilter(color), pointerEvents: "none" }} />
      </>}
...
function BloomFascia({ children, color }: { children: React.ReactNode; color: string }) {
  return (
    <div style={{ margin: "0 16px", position: "relative", borderRadius: 20, overflow: "hidden" }}>
      <img src={B.fascia} alt="" aria-hidden style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.18, filter: inkFilter(color) }} />
      <div style={{ position: "relative", background: `${color}08`, borderRadius: 20, padding: "24px 22px" }}>
        {children}
      </div>
    </div>
  );
...
export default function InvitePage() {
  ...
  const submitRSVP = async () => {
    if (!wedding || !rsvpName || rsvpChoice === null) return;
  ...
  const submitMessage = async () => {
    ...
    await supabase.from("messages").insert({ wedding_id: wedding.id, author_name: rsvpName || guest?.first_name || "Gast", content: msgText, status: "pending" });
    setMsgSent(true); setMsgText("");
  };

  // ── Loading ──────────────────────────────────────────────────────────────
  if (!wedding) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100svh", background: "#f5ede8" }}>
  ...
  // ── FASE 1: BloomOpening (idle → video → done) ────────────────────────
  if (!opened) {
    return (
      <BloomOpening
        namen={namen}
  ...
  return (
    <div style={{ minHeight: "100svh", background: "#f5ede8", fontFamily: "serif" }}>
      <style>{`
        ...
        .bfu1 { animation-delay: 0.08s; }
        .bfu2 { animation-delay: 0.18s; }
        .bfu3 { animation-delay: 0.28s; }
        .bfu4 { animation-delay: 0.38s; }
        .bfu5 { animation-delay: 0.48s; }
        .bfu6 { animation-delay: 0.58s; }
      ...
      {/* Muziek toggle */}
      <button onClick={toggleMusic} aria-label={musicOn ? "Muziek pauzeren" : "Muziek afspelen"} style={{ position: "fixed", top: 16, right: 16, zIndex: 200, width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.95)", border: `1px solid ${color}25`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 14px rgba(0,0,0,0.10)" }}>
        <Music size={15} style={{ color: musicOn ? color : "#9a8e88" }} />
      </button>

      <div style={{ maxWidth: 520, margin: "0 auto", paddingBottom: 80 }}>

        {/* ═══ BLOOM HERO — originele bl-panel structuur ═══════════════════ */}
        {/* Naadloze overgang: achtergrond #f5ede8 = zelfde als video overlay */}
        <BloomHero
          namen={namen}
          datum={datumLang}
          locatie={wedding.venue}
          stad={wedding.city}
          color={color}
          immediate={false}
        />
        ...
        {/* ═══ DATUM & LOCATIE ════════════════════════════════════════════ */}
        <div className="bfu bfu1" style={{ marginTop: 24, marginBottom: 16 }}>
          <BloomCard withCornice color={color}>
            <div style={{ textAlign: "center" }}>
              ...
              {wedding.wedding_time && <p style={{ fontFamily: "sans-serif", fontSize: 13, color: "#6b6560", margin: 0 }}>Aanvang {wedding.wedding_time} uur</p>}
            </div>
            <div style={{ textAlign: "center", margin: "16px 0" }}>
              <img src={B.wave} alt="" aria-hidden style={{ width: "60%", opacity: 0.25 }} />
            </div>
            <div style={{ textAlign: "center" }}>
              ...
              {wedding.address && (
                <a href={`https://maps.google.com/?q=${encodeURIComponent(wedding.address + " " + wedding.city)}`} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 10, fontFamily: "sans-serif", fontSize: 12, color, textDecoration: "none" }}>
          ...
          <div className="bfu bfu2" style={{ marginBottom: 16 }}>
            <BloomFascia color={color}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
                {[{ v: countdown.days, l: "dagen" }, { v: countdown.hours, l: "uren" }, { v: countdown.minutes, l: "min" }, { v: countdown.seconds, l: "sec" }].map(({ v, l }) => (
          ...
          <div className="bfu bfu2" style={{ marginBottom: 16 }}>
            <BloomCard color={color}>
              <div style={{ textAlign: "center", marginBottom: 12 }}>
                <img src={B.fiocco} alt="" aria-hidden style={{ width: 36, opacity: 0.3, filter: ifilter }} />
              </div>
              <p style={{ fontFamily: "serif", fontSize: 16, fontStyle: "italic", color: "#5a5550", lineHeight: 1.8, textAlign: "center", margin: 0 }}>"{wedding.welcome_message}"</p>
              <div style={{ textAlign: "center", marginTop: 12 }}>
                <img src={B.wave} alt="" aria-hidden style={{ width: "45%", opacity: 0.22 }} />
              </div>
            </BloomCard>
          </div>
        )}
        ...
        {events.length > 0 && (
          <div className="bfu bfu3" style={{ marginBottom: 16, padding: "0 16px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {events.map(ev => (
                <div key={ev.id} style={{ background: "white", borderRadius: 16, padding: "16px 18px", boxShadow: "0 2px 14px rgba(0,0,0,0.06)", position: "relative", overflow: "hidden", borderLeft: ev.is_main ? `3px solid ${color}` : "3px solid transparent" }}>
                  ...
                  <div style={{ display: "flex", gap: 14, alignItems: "flex-start", position: "relative" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {ev.start_time && <p style={{ fontFamily: "sans-serif", fontSize: 12, color: "#9a8e88", margin: 0 }}>{ev.start_time}{ev.end_time ? ` – ${ev.end_time}` : ""}</p>}
                      {ev.venue && <p style={{ fontFamily: "sans-serif", fontSize: 12, color: "#6b6560", margin: "2px 0 0" }}>{ev.venue}{ev.city ? `, ${ev.city}` : ""}</p>}
                      {ev.description && <p style={{ fontFamily: "sans-serif", fontSize: 12, color: "#9a8e88", marginTop: 5, lineHeight: 1.5 }}>{ev.description}</p>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        {wedding.show_rsvp && (
          <div className="bfu bfu4" style={{ marginBottom: 16 }}>
            <BloomFascia color={color}>
              ...
              <p style={{ fontFamily: "serif", fontSize: 20, color: "#16161D", margin: "0 0 18px" }}>Ben jij erbij?</p>
              {rsvpStatus === "done" ? (
                ...
                <>
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <input value={rsvpName} onChange={e => setRsvpName(e.target.value)} placeholder="Jouw naam" style={{ border: "1.5px solid #e0dbd7", borderRadius: 10, padding: "10px 14px", fontFamily: "sans-serif", fontSize: 14, width: "100%", boxSizing: "border-box", outline: "none", background: "rgba(255,255,255,0.8)" }} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
                    <button onClick={() => setRsvpChoice("yes")} style={{ background: rsvpChoice === "yes" ? color : "rgba(255,255,255,0.8)", color: rsvpChoice === "yes" ? "white" : "#5a5550", border: `1.5px solid ${rsvpChoice === "yes" ? color : "#e0dbd7"}`, borderRadius: 10, padding: "11px", fontFamily: "sans-serif", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.2s" }}>
                      {rsvpChoice === "yes" && <Check size={13} />} Ik kom!
                    </button>
                    <button onClick={() => setRsvpChoice("no")} style={{ background: rsvpChoice === "no" ? "#5a5550" : "rgba(255,255,255,0.8)", color: rsvpChoice === "no" ? "white" : "#5a5550", border: `1.5px solid ${rsvpChoice === "no" ? "#5a5550" : "#e0dbd7"}`, borderRadius: 10, padding: "11px", fontFamily: "sans-serif", fontSize: 13, cursor: "pointer", transition: "all 0.2s" }}>
                      Ik kan helaas niet
                    </button>
                  </div>
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <input value={rsvpDiet} onChange={e => setRsvpDiet(e.target.value)} placeholder="Vegetarisch, allergieën..." style={{ border: "1.5px solid #e0dbd7", borderRadius: 10, padding: "10px 14px", fontFamily: "sans-serif", fontSize: 14, width: "100%", boxSizing: "border-box", outline: "none", background: "rgba(255,255,255,0.8)" }} />
                  </div>
                  <button onClick={submitRSVP} disabled={!rsvpName || rsvpChoice === null} style={{ background: (!rsvpName || rsvpChoice === null) ? "#c0b8b4" : color, color: "white", border: "none", borderRadius: 999, padding: "13px", fontFamily: "sans-serif", fontSize: 14, fontWeight: 600, cursor: (!rsvpName || rsvpChoice === null) ? "default" : "pointer", width: "100%", transition: "background 0.2s" }}>Bevestigen</button>
                </>
              )}
            </BloomFascia>
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", margin: "20px 0 12px" }}>
          <img src={B.colomba} alt="" aria-hidden style={{ width: 44, opacity: 0.22, filter: ifilter }} />
        </div>
          ...
          <div className="bfu bfu5" style={{ marginBottom: 16 }}>
            <BloomCard color={color}>
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ fontFamily: "sans-serif", fontSize: 13, color: "#9a8e88", margin: "0 0 16px", lineHeight: 1.5 }}>Upload jouw favoriete moment van deze dag</p>
                <label style={{ display: "inline-block", background: `${color}12`, color, border: `1.5px solid ${color}35`, borderRadius: 999, padding: "10px 22px", fontFamily: "sans-serif", fontSize: 13, cursor: "pointer" }}>
                  Foto kiezen
            ...
            <BloomCard color={color}>
              ...
              {msgSent ? (
                <div style={{ textAlign: "center", padding: "16px 0" }}>
                  <img src={B.fiocco} alt="" aria-hidden style={{ width: 32, opacity: 0.3, filter: ifilter, margin: "0 auto 8px", display: "block" }} />
                  <p style={{ fontFamily: "serif", fontSize: 16, color: "#5a5550", margin: 0 }}>Bedankt voor je bericht!</p>
                </div>
              ) : (
                <>
                  <textarea rows={3} value={msgText} onChange={e => setMsgText(e.target.value)} placeholder="Schrijf een persoonlijk bericht voor het bruidspaar..." style={{ border: "1.5px solid #e0dbd7", borderRadius: 12, padding: "11px 14px", fontFamily: "sans-serif", fontSize: 13, width: "100%", boxSizing: "border-box", outline: "none", resize: "vertical", marginBottom: 10 }} />
                  <button onClick={submitMessage} disabled={!msgText.trim()} style={{ background: msgText.trim() ? color : "#c0b8b4", color: "white", border: "none", borderRadius: 999, padding: "11px 20px", fontFamily: "sans-serif", fontSize: 13, cursor: msgText.trim() ? "pointer" : "default", width: "100%", transition: "background 0.2s" }}>Bericht sturen</button>
                </>
              )}
            </BloomCard>
          </div>
        ...
        <div style={{ textAlign: "center", padding: "28px 16px 0" }}>
          <div style={{ position: "relative", margin: "0 16px 20px", borderRadius: 12, overflow: "hidden", height: 40 }}>
            <img src={B.fascia} alt="" aria-hidden style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.22, filter: ifilter }} />
          </div>
          <img src={B.fiocco} alt="" aria-hidden style={{ width: 36, opacity: 0.25, filter: ifilter, margin: "0 auto 10px", display: "block" }} />
          <img src={B.wave} alt="" aria-hidden style={{ width: "40%", opacity: 0.2, margin: "0 auto 10px", display: "block" }} />
